...
платників і отримувачів (за винятком, коли проводиться [пошук трансакцій, 
оприлюднених за попередній день](пошук-трансакцій-за-попередній-день)):
...
Скрипт вивантажує дані лише у файл одного формату у випадках, якщо обрано формат JSON або CSV. 
Для формату SQL зроблено [виняток](#%D0%97%D0%B1%D0%B5%D1%80%D0%B5%D0%B6%D0%B5%D0%BD%D0%BD%D1%8F-json-%D0%BF%D1%80%D0%B8-%D0%B2%D0%B8%D0%B2%D0%B0%D0%BD%D1%82%D0%B0%D0%B6%D0%B5%D0%BD%D0%BD%D1%96-%D0%B4%D0%BE-sqlite).
...
при роботі з API версії 2.0 як для запиту, так і для відповіді використовується 
формат дати [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) `РРРР-ММ-ДД` 
(параметри `-s`, `--startdate` та `-e`, `--enddate` відповідно). 

```python
python edata.py -p 00130850 13648033 -j -s 2015-02-01 -e 2015-10-03
```

У випадку, якщо діапазон дат не вказано, діапазон дорівнюватиме 92 дням. Так, 
при запуску скрипта 16 жовтня 2016 року транзакції повернуто за 
...
Опція `-v`, `--verbose` працює [аналогічно](#Вивід-інформації) подібній опції скрипта `edata.py`, і додатково інформує, чи була створена таблиця у базі даних для додавання записів.
...
`mysqlite.sqlite` та виводить інформацію.
### TODO ###
#### edata.py ####
- [x] конвертувати ISO 8601 datetime у ISO 8601 date
- [ ] додати завантаження переліку ЄДРПОУ із файлів
- [x] додати обробку параметра `lastload`, який повертає дату повного завантаження транзакцій (які іноді додаються частинами протягом дня)
...
`extractor.py` — це **simple downloader** для щоденного завантаження ZIP-архівів із транзакціями за вказаний період. Скрипт проходить по кожній даті в заданому діапазоні, викликає внутрішню процедуру `transactions` і зберігає результат у файл виду `YYYY-MM-DD.zip`. Якщо каталог для збереження ще не існує, він створюється автоматично.

### Що робить

- приймає **початкову дату** як обов’язковий аргумент;
- може приймати **кінцеву дату** через `-ed`;
- якщо кінцеву дату не вказано, використовує **останній день місяця** для `start_date`;
- завантажує **по одному ZIP-файлу на кожну дату** в діапазоні;
- зберігає файли до каталогу `edata/data/`;
- у режимі `--verbose` виводить дати, які обробляє.

### Використання

```bash
python -m edata.extractor 2026-03-01
python -m edata.extractor 2026-03-01 -ed 2026-03-10
python -m edata.extractor 2026-03-01 -ed 2026-03-10 --verbose
```

### Аргументи

- `start_date` — початкова дата у форматі `YYYY-MM-DD`;
- `-ed` — кінцева дата у форматі `YYYY-MM-DD`;
- `-v`, `--verbose` — виводити дати під час обробки.

### Особливості

- для кожної дати скрипт формує окремий запит, задаючи однакові `startdate` і `enddate`;
- між завантаженнями робить невелику паузу `1.5` секунди;
- у разі помилки формату дати скрипт завершується з повідомленням про те, що очікується формат ISO 8601.